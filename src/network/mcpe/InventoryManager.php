...
class InventoryManager implements InventoryListener{
	...
	private function sendInventoryContentPackets(int $windowId, array $itemStackWrappers) : void{
		...
		//now send the real contents
		$this->session->sendDataPacket(InventoryContentPacket::create($windowId, $itemStackWrappers, new FullContainerName($this->lastWindowNetworkId), 0, new ItemStackWrapper(0, ItemStack::null())));
	}

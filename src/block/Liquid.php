...
abstract class Liquid extends Transparent{
		...
		if($this->isSource() || !$bottomBlock->canBeFlowedInto()){
			...
			if($adjacentDecay <= self::MAX_DECAY){
				...
				foreach($calculator->getOptimalFlowDirections($x, $y, $z) as $facing){
					if(!$this->isSideAvailable($owningBlock, Facing::from($facing))){
						continue;
					}
	...
	/** @phpstan-impure */
	private function getSmallestFlowDecay(Block $block, int $decay, Facing $face) : int{
	...
	protected function isSideAvailable(Block $block, Facing $face) : bool{
		return true;
	}
